...
namespace planTopia.Enemies
{
    ...
    {
        [SerializeField] private ShootingAttributes shootingAttributes;

        [SerializeField] private ParticleSystem ParticleSystemStart;
            ...
            if (Time.time > NextRate && !Player.GetComponent<PlayerHealth>().isDeath)
            {
                ...
                if (Physics.Raycast(ray, out hitInfo))
                {
                    if (hitInfo.collider.gameObject.tag == "Player")
                        hitInfo.collider.gameObject.GetComponent<PlayerHealth>()
                            ?.DecreaseHealth(shootingAttributes.Damage);
